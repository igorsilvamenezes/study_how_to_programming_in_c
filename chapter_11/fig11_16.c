...
int main( void )
{
	...
	int choice; /* user's choise */
	
	/* fopen opens the file; exits if file cannot be openned */
		...
		while ( ( choice = enterChoice() ) != 5 ) {
			switch( choice ) {
				...
				case 4:
					deleteRecord( cfPtr );
					break;
				/* display message if user does not select valir choice */
				default:
...
int enterChoice( void )
{
	...
	printf( "\nEnter your choice\n"
		...
		"2 - update an account\n"
		"3 - add a new accounts\n"
		"4 - delete an account\n"
		"5 - end program\n? " );
		
	scanf("%d", &menuChoice ); /* receive choice from user */
	return menuChoice;	
} /* end function menuChoice */
